		...
		<div class="detail_view">
			...
			<div class="info_item">
				...
				<div class="btn_view">
					...
					<el-button class="edit_btn" v-if="centerType&&btnAuth('restaurant_info','修改')" type="primary" @click="editClick">修改</el-button>
					<el-button class="del_btn" v-if="centerType&&btnAuth('restaurant_info','删除')" type="danger" @click="delClick">删除</el-button>
				</div>
			</div>
		</div>
	</div>
...
<script setup>
	...
	const title = ref('')
	const detail = ref({})
	const reservationDisabled = computed(()=>detail.value?.table_status === '禁止预约' || detail.value?.table_status === '已预约')
	...
	const getDetail = (id) => {
		context?.$http({
            ...
            bannerList.value = res.data.data.cover_image?res.data.data.cover_image.split(','):[]
            title.value = res.data.data.seat_name
	...
	const restaurantReservationOnAcross = async () => {
		if(!context?.$toolUtil.storageGet('frontToken')){
		...
		context?.$toolUtil.storageSet('crossStatusColumnName','')
		context?.$toolUtil.storageSet('crossTips','')
		context?.$toolUtil.storageSet('crossStatusColumnValue','')
		nextTick(()=>{
			router.push(`/index/restaurant_reservationAdd?type=cross&&id=${detail.value.id}`)
		})
	}
	onMounted(()=>{
		init()
