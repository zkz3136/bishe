	...
	<div class="list-page" :style='{}'>
		...
		<el-form :inline="true" :model="searchQuery" class="list_search">
			...
			<div class="search_btn_view">
				...
				<el-button class="add_btn" type="success" v-if="btnAuth('restaurant_info','新增')" @click="addClick">新增</el-button>
			</div>
		</el-form>
			...
			<div class="data_box">
				<div class="data_view">
					<div class="data_item" v-for="(item,index) in list" :key="index" @click.stop="detailClick(item.id)" >
						<div class="data_img_box" v-if="item.cover_image && item.cover_image.substr(0,4)=='http'" @click.stop="preViewClick(item.cover_image)">
							<el-image class="data_img" :src="item.cover_image" fit="cover"></el-image>
						</div>
						<div class="data_img_box" v-else @click.stop="preViewClick($config.url+item.cover_image.split(',')[0])">
							<el-image class="data_img" :src="item.cover_image?$config.url + item.cover_image.split(',')[0]:''"
								fit="cover"></el-image>
						</div>
						<div class="data_content">
							<div class="data_title1">
								<span>{{item.seat_name}}</span>
							</div>
							<div class="data_title2">
								<span>可坐人数：{{item.capacity}}</span>
							</div>
						</div>
					</div>
				</div>
...
<script setup>
	...
	const addClick = () => {
		router.push('/index/restaurant_infoAdd')
	}
	//判断是否从个人中心跳转
		...
		context?.$http({
            ...
            if (start && end) {
                context?.$http({
                    ...
                    rawList = rawList.filter(i=>!reservedSeats.has(i.seat_name))
                    list.value = rawList
                    total.value = Number(rawList.length)
