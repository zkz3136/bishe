	...
	<div class="list-page" :style='{}'>
		...
		<el-form :inline="true" :model="searchQuery" class="list_search">
			...
			<div class="search_btn_view">
				...
				<el-button class="add_btn" type="success" v-if="btnAuth('restaurant_event','新增')" @click="addClick">新增</el-button>
			</div>
		</el-form>
		<div class="page_list">
			<div class="data_box">
				<div class="data_view">
					<div class="data_item" v-for="(item,index) in list" :key="index" @click.stop="detailClick(item.id)" >
						<div class="data_img_box" v-if="item.event_image && item.event_image.substr(0,4)=='http'" @click.stop="preViewClick(item.event_image)">
							<el-image class="data_img" :src="item.event_image" fit="cover"></el-image>
						</div>
						<div class="data_img_box" v-else @click.stop="preViewClick($config.url+item.event_image.split(',')[0])">
							<el-image class="data_img" :src="item.event_image?$config.url + item.event_image.split(',')[0]:''"
								fit="cover"></el-image>
						</div>
...
<script setup>
	...
	const addClick = () => {
		router.push(`/index/${tableName}Add`)
	}
	//判断是否从个人中心跳转
