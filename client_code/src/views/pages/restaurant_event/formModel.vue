		...
		<div class="detail_view">
			...
			<div class="info_view">
				...
				<div class="info_item">
					...
					<div  class="info_text" >{{detail.publish_time}}</div>
				</div>
			<div class="btn_view">
				<el-button class="edit_btn" v-if="centerType&&btnAuth('restaurant_event','修改')" type="primary" @click="editClick">修改</el-button>
				<el-button class="del_btn" v-if="centerType&&btnAuth('restaurant_event','删除')" type="danger" @click="delClick">删除</el-button>
			</div>
			</div>
		</div>
		<el-tabs type="border-card" v-model="activeName" class="tabs_view">
			<el-tab-pane label="活动内容" name="first">
				<div v-html="detail.event_content"></div>
			</el-tab-pane>
		</el-tabs>
	</div>
</template>
<script setup>
	...
	const getDetail = (id) => {
		context?.$http({
            ...
            bannerList.value = res.data.data.event_image?res.data.data.event_image.split(','):[]
            title.value = res.data.data.event_name
			startTime.value = new Date().getTime()
			endTime.value = new Date(res.data.data.end_time).getTime()
	...
	const downClick = (file) => {
		...
		const arr = file.replace(new RegExp('file/', "g"), "")
        const url = context?.$config.url + 'file/' + arr
        window.open(url)
	}
	// 判断是否从个人中心跳转
