	...
	<div class="list-page" :style='{}'>
		...
		<el-form :inline="true" :model="searchQuery" class="list_search">
			...
			<div class="search_btn_view">
				...
				<el-button class="add_btn" type="success" v-if="btnAuth('restaurant_reservation','新增')" @click="addClick">新增</el-button>
			</div>
		</el-form>
							...
							<template #default="scope">
								<div v-if="scope.row.cover_image">
									<el-image v-if="scope.row.cover_image.substring(0,4)=='http'" preview-teleported
										...
										:src="scope.row.cover_image.split(',')[0]" style="width:100px;height:100px" @click.stop></el-image>
									<el-image v-else preview-teleported
										:preview-src-list="[$config.url+scope.row.cover_image.split(',')[0]]"
										:src="$config.url+scope.row.cover_image.split(',')[0]" style="width:100px;height:100px" @click.stop>
									</el-image>
								</div>
								<div v-else>无图片</div>
							</template>
...
<script setup>
	...
	const addClick = () => {
		router.push('/index/restaurant_reservationAdd')
	}
	//判断是否从个人中心跳转
