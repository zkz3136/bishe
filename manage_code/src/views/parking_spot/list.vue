<template>
	<div>
		<div class="center_view">
			...
			<div class="dataList"
				 v-if="btnAuth('parking_spot','查看')"
			>
				<div class="item" v-for="(item,index) in list" :key="item.id">
                    <template v-if="item.spot_image">
                        <el-image v-if="item.spot_image.substring(0,4)=='http'" preview-teleported
                                  :preview-src-list="[item.spot_image.split(',')[0]]"
                                  :src="item.spot_image.split(',')[0]"></el-image>
                        <el-image v-else preview-teleported
                                  :preview-src-list="[$config.url+item.spot_image.split(',')[0]]"
                                  :src="$config.url+item.spot_image.split(',')[0]">
                        </el-image>
                    </template>
					...
					<div class="btns">
						...
						<el-button class="operate_btn" type="warning" @click="exitClick(item.id)" v-if="item.spot_status=='已使用'">
							离场
						</el-button>
...
<script setup>
	...
	// 入场
	const entryClick = (id) => {
		ElMessageBox.prompt('请输入车牌号', '入场', {
			confirmButtonText: '确定',
			cancelButtonText: '取消',
			inputPattern: /^[\u4e00-\u9fa5]{1}[A-Z]{1}[A-Z_0-9]{5}$/,
	...
	// 离场
	const exitClick = (id) => {
		ElMessageBox.confirm('确认离场并结算费用？', '提示', {
			confirmButtonText: '确定',
			cancelButtonText: '取消',
			type: 'warning'
		}).then(() => {
			context.$http({
				...
				ElMessageBox.alert(`停车时长：${duration}分钟，费用：${price}元`, '结算', {
					confirmButtonText: '确定',
					callback: action => {
						getList()
					}
