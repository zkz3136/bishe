...
<script setup>
	...
	const init=(formId=null,formType='add',formNames='',row=null,table=null,statusColumnName=null,tips=null,statusColumnValue=null)=>{
		...
		if(formType == 'add'){
			...
			for(let x in row){
				...
				if(x=='avatar_url'){
					form.value.avatar_url = row[x];
					disabledForm.value.avatar_url = true;
					continue;
				}
				if(x=='nickname'){
	...
	const save= async ()=>{
		...
		if(form.value.avatar_url!=null) {
			form.value.avatar_url = form.value.avatar_url.replace(new RegExp(context?.$config.url,"g"),"");
		}
